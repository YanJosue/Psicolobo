...
def cargar_diccionario_emociones(ruta_csv='baseConocimientos/emociones_v2.csv'):
    ...
    try:
        ...
        df = pd.read_csv(ruta_csv, encoding='utf-8', sep=',') # <<<< ESTE ES EL PARÁMETRO CLAVE

        # --- LÍNEA DE DEPURACIÓN CRUCIAL (para ver qué columnas lee Pandas) ---
        ...
        for p_token in processed_tokens: # Iterar sobre los tokens procesados
            ...
            for idx, row in diccionario_df.iterrows():
                ...
                if p_token['is_negated'] and negacion_considerar_str == 'sí':
                    # Podrías agregar un debug aquí si quieres ver cuáles se están ignorando por negación
...
if __name__ == "__main__":
    ...
    for i, texto in enumerate(ejemplos_texto):
        ...
        print(f"  Conteo de emociones: {dict(conteos)}") # ¡CAMBIO AQUÍ! Usar 'conteos'
        print(f"  Términos que contribuyeron: {dict(terminos_encontrados)}")

        # Aquí podrías integrar la lógica de respuesta de Oscova
        if emocion == "NO_DETECTADA" or emocion == "EMPATE":
            ...
            # Recuperar una respuesta del diccionario (si es relevante, o pasar a Oscova)
            ...
            if palabra_ejemplo_detectada:
                ...
                # Asegúrate que 'Termino_emocional' y 'Respuesta' sean las claves correctas aquí
